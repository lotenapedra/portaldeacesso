...
def obter_municipios(estado):
    ...
    with open(arquivo_csv, 'r', newline='', encoding='latin-1') as file:
...
def validate_form():
    ...
    mandatory_fields = [nome_completo, tipo_veiculo, motivo, placa, estado_origem, cidade_origem, empresa_origem, telefone, frete_retono]

    # Dictionary to hold labels and their corresponding values for displaying missing fields in error message
    labels = {
        ...
        empresa_origem: "Empresa de Origem",
        telefone: "Telefone",
        frete_retono: "Possuem Frete Retorno?"
    }

    # List to store missing fields
    missing_fields = [labels[field] for field in mandatory_fields if not field]

    # Display error message if any mandatory field is missing
    if missing_fields:
        missing_fields_str = ", ".join(missing_fields)
        st.error(f"Os seguintes campos são obrigatórios: {missing_fields_str}")
        return False

    return True

if st.button("Salvar"):
    # Call the validation function before saving the data
    if validate_form():
